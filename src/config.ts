...
export function getConfig(): SocialSyncsConfig {
  ...
  const apiKey = process.env.SOCIALSYNCS_API_KEY;
  const apiUrl = process.env.SOCIALSYNCS_API_URL;

  if (!apiKey) {
    console.error('❌ Error: No authentication found.');
    console.error('Options:');
    console.error('  1. OAuth2: socialsyncs auth:login');
    console.error('  2. API Key: export SOCIALSYNCS_API_KEY=your_api_key');
    process.exit(1);
  }
