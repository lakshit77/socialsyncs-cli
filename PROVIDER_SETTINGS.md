...
The SocialSyncs CLI supports platform-specific settings for each integration. Different platforms have different options and requirements.
...
```bash
socialsyncs posts:create --json post-with-settings.json
```
...
```bash
socialsyncs posts:create --json reddit-post.json
```
...
```bash
socialsyncs posts:create --json youtube-video.json
```
