# SocialSyncs CLI - Advanced Examples

This directory contains examples demonstrating the full capabilities of the SocialSyncs CLI, including posts with comments and multiple media.

## Understanding the Post Structure

The SocialSyncs API supports a rich post structure:
...
```bash
socialsyncs posts:create --json examples/post-with-comments.json
```
...
```bash
socialsyncs posts:create --json examples/multi-platform-post.json
```
...
```bash
socialsyncs posts:create --json examples/thread-post.json
```
...
for file in posts/*.json; do
  ...
  socialsyncs posts:create --json "$file"
  sleep 2
done
```
...
socialsyncs posts:create --json thread.json
```
...
2. **Invalid image path** - Ensure images are accessible URLs or uploaded to SocialSyncs first
