...
const CLIENT_ID = process.env.SOCIALSYNCS_OAUTH_CLIENT_ID!;
const CLIENT_SECRET = process.env.SOCIALSYNCS_OAUTH_CLIENT_SECRET!;
const FRONTEND_URL = process.env.SOCIALSYNCS_FRONTEND_URL || 'https://app.socialsyncs.co';
const API_URL = process.env.SOCIALSYNCS_API_URL || 'https://app.socialsyncs.co/api';
const SERVER_URL = process.env.SERVER_URL || `http://localhost:${PORT}`;
const DATABASE_URL = process.env.DATABASE_URL!;

if (!CLIENT_ID || !CLIENT_SECRET) {
  console.error('SOCIALSYNCS_OAUTH_CLIENT_ID and SOCIALSYNCS_OAUTH_CLIENT_SECRET are required');
  process.exit(1);
}
...
function html(res: ServerResponse, status: number, body: string) {
  ...
  res.end(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SocialSyncs CLI Auth</title>
...
function handleVerifyPage(req: IncomingMessage, res: ServerResponse) {
  ...
  html(res, 200, `
    <h2>SocialSyncs CLI Authorization</h2>
...
async function handleVerifySubmit(req: IncomingMessage, res: ServerResponse) {
  ...
  // Redirect to SocialSyncs OAuth with device_code in state so we can match on callback
...
// GET /device/callback — SocialSyncs redirects here after OAuth authorization
...
async function start() {
  ...
  server.listen(PORT, () => {
    console.log(`SocialSyncs CLI auth server running on ${SERVER_URL}`);
    console.log(`OAuth callback URL (configure in SocialSyncs): ${SERVER_URL}/device/callback`);
  });
}
