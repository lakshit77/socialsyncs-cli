# SocialSyncs CLI - Project Structure

## Overview

The SocialSyncs CLI is a complete command-line interface package for interacting with the SocialSyncs social media scheduling API. It's designed for developers and AI agents to automate social media posting.
...
│   ├── api.ts                    # API client for SocialSyncs API
...
#### `src/api.ts`
- API client class `SocialSyncsAPI`
- Handles all HTTP requests to the SocialSyncs API
- Methods for:
  - Creating posts
  - Listing posts
  - Deleting posts
...
- `uploadFile()` - Upload images to SocialSyncs
...
- Version: `1.0.0`
- Executable bin: `socialsyncs` → `dist/index.js`
...
|----------|----------|---------|-------|
| `SOCIALSYNCS_API_KEY` | ✅ Yes | - | Authentication token |
| `SOCIALSYNCS_API_URL` | ❌ No | `https://app.socialsyncs.co/api` | Custom API endpoint |
...
SOCIALSYNCS_API_KEY=test node dist/index.js integrations:list
```
...
2. **Features**
   - Interactive mode
   - Config file support (~/.socialsyncsrc)
...
- **Issues:** https://github.com/lakshit77/socialsyncs-cli/issues
- **Docs:** See README.md, SKILL.md, QUICK_START.md
- **Website:** https://app.socialsyncs.co
