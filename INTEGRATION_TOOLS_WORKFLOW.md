...
```bash
socialsyncs integrations:settings reddit-abc123
```
...
```bash
socialsyncs integrations:trigger reddit-abc123 getFlairs -d '{"subreddit":"programming"}'
```
...
```bash
socialsyncs integrations:settings youtube-123
```
...
```bash
socialsyncs integrations:trigger youtube-123 getPlaylists
```
...
```bash
socialsyncs integrations:settings linkedin-123
```
...
```bash
socialsyncs integrations:trigger linkedin-123 getCompanies
```
...
```bash
# No parameters
socialsyncs integrations:trigger <integration-id> <methodName>

# With parameters
socialsyncs integrations:trigger <integration-id> <methodName> -d '{"key":"value"}'
```
...
SETTINGS=$(socialsyncs integrations:settings "$INTEGRATION_ID")
...
for METHOD in $TOOLS; do
  RESULT=$(socialsyncs integrations:trigger "$INTEGRATION_ID" "$METHOD" -d '{}')
...
```bash
socialsyncs integrations:trigger reddit-123 invalidMethod
...
```bash
socialsyncs integrations:trigger reddit-123 getFlairs
...
```bash
socialsyncs integrations:trigger invalid-id getFlairs
...
FLAIRS=$(socialsyncs integrations:trigger $INTEGRATION_ID getFlairs -d '{"subreddit":"programming"}')
